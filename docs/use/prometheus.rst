...
Access the `monitoring service <http://monitor.prometheus.open-contracting.org>`__. The username is ``prom``. The password is set by the ``prometheus_server.password`` variable in the ``pillar/private/prometheus.sls`` file.
...
Access the `alerting service <http://alertmanager.prometheus.open-contracting.org>`__.  The username is ``prom``. The password is set by the ``prometheus_alertmanager.password`` variable in the ``pillar/private/prometheus.sls`` file.

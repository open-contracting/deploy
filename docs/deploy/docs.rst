...
#. In ``salt/apache/files/docs.conf.include``, add the new language in the ``options`` variable.
...
#. In ``salt/apache/files/docs.conf.include``, add the profile's latest branch, minor series and languages in the ``options`` variable.
...
#. In ``salt/apache/files/docs.conf.include``, add the minor series in the ``options`` variable, and add a new ``Location`` directive like:

ssh:
  root:
    # Young Innovations
    # Bigyan (Lead Django Developer)
    - ssh-rsa 
    # Bikram (DevOps)
    - ssh-rsa 
    # Kushal (Project Lead)
    - ssh-rsa 
    # temp-salt-server (deploys covid-19-procurement-explorer-admin)
    - ssh-rsa 
  covid19:
    # Dhondup (Frontend Developer)
    - ssh-rsa 
    # haku deploy bot (deploys covid-19-procurement-explorer-public)
    - ssh-rsa 
  covid19admin:
    # Sonika (Django Developer)
    - ssh-rsa 
    # Rubina (Django Developer)
    - ssh-rsa 
    # Suyoj (Django Developer)
    - ssh-rsa  suyoj.tamrakar@yipl.com.np

apache:
  public_access: True

python_apps:
  covid19admin:
    user: covid19admin
    git:
      url: https://github.com/open-contracting/covid-19-procurement-explorer-admin.git
      target: covid19admin
    django:
      app: covidadmin
      compilemessages: True
      env:
        DEBUG: "False"
        ALLOWED_HOSTS: admin.open-contracting.health,localhost,127.0.0.1
        CORS_ORIGIN_WHITELIST: https://open-contracting.health,https://www.open-contracting.health
        CELERY_BROKER_URL: 'pyamqp://guest@localhost/'
        CELERY_TIMEZONE: UTC
        FORCE_SCRIPT_NAME: /
        STATIC_URL: /static/
        MEDIA_URL: /media/
        FETCH_COVID_DATA_INTERVAL: '10800'
        DB_ENGINE: django.db.backends.postgresql_psycopg2
    uwsgi:
      configuration: django
      harakiri: 1800
      processes: 8
      threads: 4
      disable-logging: True
      master: true
      smart-attach-daemon: celery -A covidadmin worker -l info -Q covid19 -B
    apache:
      configuration: django
      servername: admin.open-contracting.health

react_apps:
  covid19public:
    user: covid19
    env:
      NODE_ENV: production
      REACT_APP_API_URL: https://admin.open-contracting.health
      REACT_APP_FATHOM_SRC: https://kite.open-contracting.org/script.js
      REACT_APP_FATHOM_SITE_KEY: LKQYBVCU
    git:
      url: https://github.com/open-contracting/covid-19-procurement-explorer-public.git
      target: covid19
    apache:
      configuration: react
      servername: open-contracting.health
      serveraliases: ['www.open-contracting.health']

postgres:
  version: 11
  configuration: False

nodejs:
  version: 14

vm:
  overcommit_memory: 1

ver_txt:
  enable: True
